...
@section('content')
    <div class="header">
        <h6 class="h2 mb-0 mt-3">لیست سفارش ها</h6>
                    ...
                    <div class="col-12">
                        <nav aria-label="breadcrumb" class="d-none d-md-inline-block ml-md-4">
                            <ol class="breadcrumb breadcrumb-links">
                                ...
                                <li class="breadcrumb-item active" aria-current="page">لیست سفارش ها</li>
                            </ol>
                        </nav>
                    </div>
                </div>
    ...
    <div class="col">
        <div class="card">
            <div class="card-body py-0 px-3">
                <div class="col-2 p-0 py-3 float-left">
                ...
                <div class="table-responsive">
                    <div class="table-responsive pb-4">
                        <table class="table table-flush" id="datatable">
                            ...
                            <tbody>
                            <?php $i = ($orders->currentpage() - 1) * $orders->perpage(); $i++; ?>
                            @foreach($orders as $order)
                                <tr row-id="{{$order->id}}">
                                    <td>{{\App\Helpers\Format\Number::persianNumbers($i++)}}</td>
                                    <td>{{$order->user->full_name}}</td>
                                    <td>{{$order->present()->amount}}</td>
                                    <td>{{$order->present()->created_at}}</td>
                                    <td>{!!  $order->present()->status!!}</td>
                                    <td>
                                        @can('order.show')
                                            <a class="table-action text-info" data-toggle="tooltip"
                                               data-original-title="نمایش محصولات"
                                               href="{{url('admin/orders/'.$order->id)}}"><i
                                                        class="far fa-eye   "></i></a> @endcan
                                        @if($order->status == 1)
                                            @can('order.changeStatus')
                                                <a class="table-action btn_ban text-warning" data-toggle="tooltip"
                                                   data-original-title="ارسال" row-id="{{$order->id}}"><i class="far
                                                                           fa-check"></i></a>
                                            @endcan
                                        @endif
                                        @can('order.delete')
                                            <a class="table-action btn_delete text-danger" data-toggle="tooltip"
                                               data-original-title="حذف" row-id="{{$order->id}}"><i
...
@section('javascript')
    ...
    <script>
        var table = $('#datatable').DataTable({
            ...
            "language": {
                "emptyTable": "سفارشی  برای نمایش وجود ندارد.",
        ...
        $('.navbar-search input').attr('placeholder', 'جستجو سفارش ها');
        ...
        $('.btn_delete').click(function () {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    load_screen(true);
                    $.ajax({
                        url: "{{url('admin/orders')}}/" + record_id,
                        type: "delete",
                        cache: false,
                        contentType: false,
                        processData: false,
        ...
        $('.btn_ban').click(function () {
            ...
            Swal.fire({
                title: "",
                text: "سفارش انتخابی " + btn.attr('data-original-title') + " شود؟",
                type: "question",
                showCancelButton: true,
                buttonsStyling: false,
                confirmButtonClass: "btn btn-danger m-1",
                ...
                if (result.value) {
                    load_screen(true);
                    $.ajax({
                        url: "{{url('admin/orders')}}/" + record_id + "/status",
                        type: "get",
                        dataType: "json",
                        cache: false,
                        contentType: false,
