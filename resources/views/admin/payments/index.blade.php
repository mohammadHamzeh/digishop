...
@section('content')
    <div class="header">
        <h6 class="h2 mb-0 mt-3">لیست پرداخت ها</h6>
                ...
                <div class="table-responsive">
                    <div class="table-responsive pb-4">
                        <table class="table table-flush" id="datatable">
                            ...
                            <tbody>
                            <?php $i = ($payments->currentpage() - 1) * $payments->perpage(); $i++; ?>
                            @foreach($payments as $payment)
                                <tr row-id="{{$payment->id}}">
                                    <td>{{\App\Helpers\Format\Number::persianNumbers($i++)}}</td>
                                    <td>{{$payment->orders->user->full_name}}</td>
                                    <td>{{$payment->present()->amount}}</td>
                                    <td>{{$payment->present()->method}}</td>
                                    <td>{{$payment->present()->gateway}}</td>
                                    <td>{{$payment->present()->created_at}}</td>
...
@section('javascript')
    ...
    <script>
        var table = $('#datatable').DataTable({
            ...
            "language": {
                "emptyTable": "پرداخت ی  برای نمایش وجود ندارد.",
        ...
        $('.navbar-search input').attr('placeholder', 'جستجو پرداخت  ها');
        ...
        $('.btn_ban').click(function () {
            ...
            Swal.fire({
                title: "",
                text: "پرداخت  انتخابی " + btn.attr('data-original-title') + " شود؟",
                type: "question",
                showCancelButton: true,
                buttonsStyling: false,
                confirmButtonClass: "btn btn-danger m-1",
