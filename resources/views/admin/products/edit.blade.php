...
@section('content')
    <div class="header">
        <h6 class="h2 mb-0 mt-3">ویرایش محصول</h6>
                ...
                <div class="form-row mb-3">
                    <div class="col-6">
                        ...
                        <input type="text" class="form-control form-control-alternative price_number" name="price"
                               placeholder="قیمت به تومان..." value="{{number_format($product->price)}}">
                    </div>
                    <div class="col-6">
                        ...
                        <input type="text" class="form-control form-control-alternative" name="stock"
                               placeholder="مثلا:۲۰ " value="{{$product->stock}}">
                    </div>
                </div>
                <div class="form-row mb-3">
                    <div class="col-12">
                    ...
                    <div class="col-12">
                        ...
                        <select name="ProductStatus" id="ProductStatus" class="form-control">
                            @foreach($productStatus as $key=>$value)
                                <option value="{{$key}}" {{$key==$product->status ? 'checked':''}}>{{$value}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="form-row mb-3">
                    <div class="col-12">
                        ...
                        <input type="text" class="form-control form-control-alternative" name="meta_data_title"
                               placeholder="" value="{{optional($product->meta_tag)->title}}">
                    </div>
                </div>
                <div class="form-row mb-3">
                    <div class="col-12">
                        ...
                        <input type="text" class="form-control form-control-alternative" name="meta_data_description"
                               placeholder="" value="{{optional($product->meta_tag)->description}}">
                    </div>
                </div>
                <div class="form-row mb-3">
                    <div class="col-12">
                        ...
                        <input type="text" class="form-control" data-toggle="tags" name="meta_data_keyword"
                               value="{{optional($product->meta_tag)->keyword}}"/></div>
                </div>
                <div class="form-row mb-3">
                    <div class="col-12">
                        ...
                        <input type="text" class="form-control form-control-alternative" name="meta_data_author"
                               placeholder="" value="{{optional($product->meta_tag)->author}}">
                    </div>
                </div>
            </div>
...
@section('javascript')
    ...
    <script>
        ...
        $(document).ready(function () {
            var image = "<?=url('storage/' . $product->image)?>";
            var imageSeo = "<?=url('storage/' . $product->meta_tag->image)?>";

            $('input[name="product_img"]').closest('.img_upload').children('img').attr('src', image);
            $('input[name="meta_data_image"]').closest('.img_upload').children('img').attr('src', imageSeo);

            $('#summernote').summernote('code', '<?=$product->text?>');
        });
        ...
        $('.btn_edit_confirm').click(function () {
            ...
            var ProductStatus = $('.card-body select[name="ProductStatus"]').val();
